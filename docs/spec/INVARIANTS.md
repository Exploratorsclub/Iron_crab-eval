...
- **On-Wire E2E (schmaler Vertrag):** `request_reply_e2e_pumpswap_intent_pool_hint_on_control_request` nutzt denselben **manuellen Cold-Path-Wire** wie `trade_intent_manual_sell_all_pumpfun_route_roundtrip` (Eval): `source=ui-manual`, `metadata.sell_all=true`, `metadata.dex=pump_amm` (PumpSwap statt `pumpfun`), plus MOMENTUM_V2_SPEC §10.2 (14er `resources.accounts`, `accounts[0]==pools[0]`). Publiziert auf `ironcrab.v1.trade_intents` und erwartet auf `ironcrab.v1.control_requests` ein `EnsurePumpAmmPoolAccounts` mit gleichem `base_mint` und `pool_address_hint == resources.pools[0]`, danach terminale korrelierte `ControlResponse` von `market-data`. Kein Claim ueber Sim-Fail-Trigger oder allgemeinere SELL-Pfade ohne dieses manuelle Sell-All-Shape.
- **Blackbox-Grenze:** Der E2E-Vertrag belegt Intent → ControlRequest → ControlResponse fuer diesen **manuellen Sell-All-PumpSwap**-Einstieg; feinere engine-interne Reihenfolgen bleiben ausserhalb dieses Schnitts nicht separat beobachtbar.
- **Getestet:** i24e_force_refresh_skips_stale_livepool_cache_pool_accounts; i24e_force_refresh_refuses_without_cold_path_rpc_permission; i24e_pool_address_hint_parse_fail_errors_without_global_scan; control_request_ensure_pump_amm_pool_accounts_force_refresh_and_pool_hint_roundtrip; request_reply_e2e_pumpswap_intent_pool_hint_on_control_request.
