...
- **E2E Wire-Slice (manueller PumpSwap-sell-all-Cold-Path, schmal):** Fuer den in CI/Sibling-E2E verwendeten oeffentlichen Shape `source=sell-all` mit `metadata.sell_all=true`, `metadata.dex=pump_amm`, optional konsistente Metadaten `token_account` / `token_program` (base58), mindestens einem `resources.accounts`-Eintrag (Quell-ATA), `resources.token_program` gesetzt (SPL Token laut `TradeResources`-Rustdoc), genau einem `resources.pools[0]` und SELL Token→WSOL muss auf der Control-Plane ein `EnsurePumpAmmPoolAccounts` erscheinen mit `base_mint == resources.input_mint` und `pool_address_hint == resources.pools[0]`, gefolgt von einer korrelierten terminalen `ControlResponse` (`ok` \| `not_found` \| `error`) mit `target=market-data`. Getestet in `request_reply_e2e_manual_pumpswap_sell_all_pool_hint_roundtrip`. Kein Claim ueber allgemeine SELL-Pfade, Hot-Path-Recovery oder vollstaendige Engine-Orchestrierung — nur dieser Intent→ControlRequest→ControlResponse-Schnitt.
...
- **Kontext:** PR #31 Merge auf architecture-rebuild; manueller sell_all PumpSwap Cold-Path Recovery auf Request/Reply (PR #65 architecture-rebuild); Hot-Path-async-Refresh fuer reguläre Sells bleibt explizit out-of-scope (kein Overclaim in diesen Tests).
