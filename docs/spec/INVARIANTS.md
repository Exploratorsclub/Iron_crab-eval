...
- **E2E Wire-Slice (PumpSwap Cold-Path vor build_tx_plan, PR #67, schmal):** Fuer den merged engen Shape: Cold-Path-Recovery-Sell (`metadata.sell_all=true` reicht als Kennzeichnung), `metadata.dex=pump_amm`, explizites `resources.pools[0]`, **`resources.accounts` leer** (pre-plan bounded `EnsurePumpAmmPoolAccounts`, wenn die SLAVE-Hint-Pool-Zeile fuer den tx_builder unzureichend ist). Publish auf `ironcrab.v1.trade_intents`; beobachtbar muss ein `EnsurePumpAmmPoolAccounts` mit `base_mint == resources.input_mint` und `pool_address_hint == resources.pools[0]` erscheinen, gefolgt von einer korrelierten terminalen `ControlResponse` (`ok` \| `not_found` \| `error`) mit `target=market-data`. Getestet in `request_reply_e2e_manual_pumpswap_sell_all_pool_hint_roundtrip` (Quelle im Test: `sell-all`; kein Claim, dass andere `source`-Werte denselben Slice triggern). Kein Claim ueber allgemeine SELL-Pfade oder vollstaendige Orchestrierung — nur dieser Intent→ControlRequest→ControlResponse-Schnitt.
