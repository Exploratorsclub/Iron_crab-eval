...
- **Beobachtbarer Vertrag (Eval):** (a) Fehlende pool_accounts fuehren nicht zu lokaler Engine-Truth-Heilung; Cache-Postcondition: SLAVE Cache unveraendert. (b) Autoritativer PoolCacheUpdate macht den Zustand verfuegbar. (c) Nach autoritativem Update kann der naechste Versuch fortfahren (14 pool_accounts). (d) not_found (Cache-Miss, allow_rpc_on_miss=false) und externer Fehler (pool in Cache, leere pool_accounts, allow_rpc_on_miss=true, RPC unreachable) sind getrennt modelliert.
...
- **Hinweis:** Echter Request/Reply-E2E zu market-data ist im Eval-Repo nicht modellierbar; externer Fehler wird ueber RPC unreachable approximiert.
