...
- **On-Wire E2E (schmaler Vertrag):** `request_reply_e2e_pumpswap_intent_pool_hint_on_control_request` publiziert einen manuellen PumpSwap-Intent (`metadata.dex=pump_amm`, 14er `resources.accounts`, `accounts[0]==pools[0]` laut MOMENTUM_V2_SPEC §10.2) auf `ironcrab.v1.trade_intents` und erwartet auf `ironcrab.v1.control_requests` ein `EnsurePumpAmmPoolAccounts` mit gleichem `base_mint` und `pool_address_hint == resources.pools[0]`, danach terminale korrelierte `ControlResponse` von `market-data`. Kein Claim ueber Sim-Fail-Trigger oder innere Priorisierung jenseits dieses NATS-/Wire-Vertrags.
- **Blackbox-Grenze:** Der E2E-Vertrag belegt Intent → ControlRequest → ControlResponse; feinere engine-interne Reihenfolgen (Cache vs. Hint vor einem Ensure) bleiben nicht separat beobachtbar ausserhalb der oeffentlichen DEX-API und dieses Wire-Schnitts.
