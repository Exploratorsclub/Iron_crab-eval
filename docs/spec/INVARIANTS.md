...
- **Source-Contract (PR233 + Phase 2a):** `tests/invariants_market_data_tracking_single_writer.rs` — `md_state_command_includes_flush_geyser_sync`; `phase2a_geyser_sync_on_track_worker_not_md_state_loop`; `global_ingest_liveness_os_thread`; `touch_tracked_vault_o1_contract`.
...
- **Getestet:** `i4b_jsonl_full_queue_try_enqueue_json_returns_immediately`; `i4b_jsonl_full_queue_try_enqueue_market_event_returns_immediately`; `i4b_jsonl_bounded_enqueue_delivers_when_capacity_available`; PR233 + Phase-2a Source-Contract in `invariants_market_data_tracking_single_writer.rs`; Phase-1 Source-Contract in `invariants_market_data_i4b_ingest_no_tracked_read.rs`.
- **Kontext:** `plan_market_data_ingest_rebuild.md` Phase R1 (JSONL `QueuedJsonlMsg::MarketEvent`) + R2 (`md-state` Thread) + PR233 (Debounce → md-state enqueue) + Phase 2a Hybrid (`plan_hybrid_rollback_tracking_architecture_20260623.md` §2a Track-Worker, §5.2 Snapshot-Ingest) + Phase 1 Hybrid (§5.4 Register-Verbot parse-Pfad).
