...
- **E2E Wire-Slice (manueller PumpSwap-Cold-Path, schmal):** Fuer den beobachtbaren Shape `source=ui-manual` mit `metadata.sell_all=true`, `metadata.dex=pump_amm`, genau einem `resources.pools[0]` und SELL Token→WSOL muss auf der Control-Plane ein `EnsurePumpAmmPoolAccounts` erscheinen mit `base_mint == resources.input_mint` und `pool_address_hint == resources.pools[0]`, gefolgt von einer korrelierten terminalen `ControlResponse` (`ok` \| `not_found` \| `error`) mit `target=market-data`. Getestet in `request_reply_e2e_manual_pumpswap_sell_all_pool_hint_roundtrip`. Kein Claim ueber allgemeine SELL-Pfade, Hot-Path-Recovery oder vollstaendige Engine-Orchestrierung — nur dieser Intent→ControlRequest→ControlResponse-Schnitt.
