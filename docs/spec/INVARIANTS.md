...
- **Invarianten:**
  ...
  - **PumpSwap Build IX — Fee-Metas (pool_accounts v1):** Für `pool_accounts` der Länge 14 (Reihenfolge laut `PumpFunAmmDex::build_swap_ix_from_pool_accounts`-Doku) bezieht sich der Vertrag auf die **Account-Metas der erzeugten PumpSwap-Swap-Instruction an den dort dokumentierten Meta-Positionen** (öffentliche API-Doku: u. a. Meta **#9** `protocol_fee_recipient`, Meta **#10** `protocol_fee_recipient_ta`, bezogen auf die beobachteten Slots **`pool_accounts[6]`** / **`pool_accounts[7]`**). Nur Builder-/Connector-Vertrag, keine Aussage über Discovery oder Market-Data-Pipeline:
    - Sind `[6]` und `[7]` beobachtete (nicht-default) Pubkeys, müssen die entsprechenden Account-Metas in der Swap-IX an diesen dokumentierten Positionen genau diese Werte widerspiegeln (kein stiller Ersatz durch einen globalen Mainnet-Recipient).
...
- **On-Wire Contract (I-24d Raydium AMM v4):** `tests/request_reply_e2e_contract.rs`: EnsureRaydiumAmmPoolState (base_mint) → market-data → korrelierte Response. Gleicher bounded Poll und request_id-Korrelation; terminaler Outcome (ok|not_found|error). Beweist nur den Raydium-AMM-v4 Cold-Path Request/Reply-Contract; kein Claim ueber andere DEX-Pfade ausser dem explizit genannten Orca-Eintrag unten.
- **On-Wire Contract (I-24d Orca Whirlpool):** `tests/request_reply_e2e_contract.rs`: EnsureOrcaWhirlpoolPoolState (base_mint) → market-data → korrelierte Response. Gleicher bounded Poll auf `TOPIC_CONTROL_RESPONSES`, Filter nach `request_id`, `target == "market-data"`, terminaler Status (ok|not_found|error). Absichtlich nicht aufloesbare `base_mint` belegt den Wire-Vertrag ohne Mainnet-Discovery. Kein Claim ueber Orca-Quote-/Builder-Semantik, Hot-Path-Caches oder vollstaendige Recovery — nur der korrelierte Request/Reply-Pfad fuer diesen ControlRequestKind.
- **On-Wire Contract (I-24d Meteora DLMM):** `tests/request_reply_e2e_contract.rs`: EnsureMeteoraDlmmPoolState (base_mint) → market-data → korrelierte Response. Gleicher bounded Poll auf `TOPIC_CONTROL_RESPONSES`, Filter nach `request_id`, `target == "market-data"`, terminaler Status (ok|not_found|error). Absichtlich nicht aufloesbare `base_mint` belegt den On-Wire-Vertrag ohne Mainnet-Discovery. Kein Claim ueber Meteora-DLMM-Quote-/Builder-Semantik, vollstaendige Cold-Path-Recovery oder andere DEX-Pfade — nur der korrelierte Request/Reply-Pfad fuer diesen ControlRequestKind.
- **On-Wire Contract (I-24d PumpFun Bonding Curve):** `tests/request_reply_e2e_contract.rs`: EnsurePumpfunBondingCurve (base_mint) → market-data → korrelierte Response. Gleicher bounded Poll auf `TOPIC_CONTROL_RESPONSES`, Filter nach `request_id`, `target == "market-data"`, terminaler Status (ok|not_found|error). Absichtlich nicht aufloesbare `base_mint` belegt den Request/Reply-Wire-Vertrag ohne Mainnet-Discovery. Kein Claim ueber `force_refresh_pumpfun`/bounded Retry, JetStream-Publish-Details oder vollstaendige Bonding-Curve-Cold-Path-Recovery — nur der korrelierte terminale Response-Pfad fuer diesen ControlRequestKind (Eval-Perspektive A.38). Ergaenzender schmaler Recovery-Slice: A.44.
...
- **Dateien:** `tests/invariants_pumpswap_amm_liquidation.rs`, `tests/ipc_schema_serde.rs`, `tests/request_reply_e2e_contract.rs`
...
- **Merged Preplan-Gate (PR #67, vor erstem `build_tx_plan`):** Bounded PumpSwap-Discovery laeuft nur wenn **alle** zutreffen: kein Replay-Modus, noch kein Preplan-Versuch fuer diesen Lauf, Cold-Path-Recovery-Sell (`is_cold_path_recovery_sell`), `metadata.dex == "pump_amm"`, **`intent.resources.accounts` leer**, **`intent.resources.pools.len() == 1`**, die gehintete Pool-Zeile im SLAVE ist fuer den tx_builder noch **nicht** brauchbar. Dann: Publish `EnsurePumpAmmPoolAccounts` mit `pool_address_hint = resources.pools[0]`, bounded Warten, **ein** erneuter Plan/Sim-Durchlauf. **Folge:** Nicht-leere `resources.accounts` **verhindern** dieses Gate (erklaert Timeouts im E2E, wenn fälschlich ein Source-ATA gesetzt war).
- **E2E Wire-Slice (nur beobachtbar, schmal):** Publish `TradeIntent` auf `ironcrab.v1.trade_intents` mit oeffentlichem Shape, der das Gate **nicht ausschliesst**: SELL, `metadata.sell_all=true`, `metadata.dex=pump_amm`, genau ein `resources.pools[0]`, **`resources.accounts` leer**. **Harness-Vorbedingungen (Eval-only, sonst erreicht die Engine den Preplan-Slice nicht):** execution-engine mit geladenem Wallet (Eval: `IRONCRAB_KEYPAIR_PATH` auf Fixture-Keypair, weiterhin `--dry-run`); vor Start der EE ein JetStream-`WalletBalanceSnapshot` (`MarketEventKind::WalletBalanceSnapshot`) auf `wallet_snapshot_subject(wallet, mint)` mit ausreichendem `balance_raw` fuer den Intent, damit SELL-Preflight und LockManager nicht vor `build_tx_plan` abbrechen (`wallet_pubkey_unavailable` / `SimInsufficientBalance`). **Seed-Publish:** Eval-Harness wartet die JetStream-**Pub-Ack** ab (`async_nats` `publish` + Await auf `PublishAck`), nicht nur ein aeusseres Publish ohne Ack — sonst kann die Nachricht fuer Readback/`get_last_raw_message_by_subject` fehlen. **Zusaetzlich:** bounded Read-after-Write auf den JetStream-Stream `WALLET_SNAPSHOT` (letzte Nachricht fuer genau dieses Subject) **bevor** die EE startet — sonst kann `LastPerSubject`-Bootstrap eine leere erste Fetch-Runde sehen und der LockManager bleibt bei Token-`available=0` (symptomatisch: `SIM_INSUFFICIENT_BALANCE`). Der Test `request_reply_e2e_manual_pumpswap_sell_all_pool_hint_roundtrip` nutzt `source=sell-all` (typisches Tooling); **kein** Claim, dass ausschliesslich dieser `source` den Slice triggert. Beobachtbar: `EnsurePumpAmmPoolAccounts` mit `base_mint == resources.input_mint`, `pool_address_hint == resources.pools[0]`, danach korrelierte terminale `ControlResponse` (`ok` \| `not_found` \| `error`, `target=market-data`). Kein weiterer Claim (kein Replay-Flag im Test, keine Engine-internen Zaehler, kein genereller SELL-Pfad).
- **Blackbox-Grenze:** API-/Unit-Tests pruefen `force_refresh` und Hint-Semantik ohne vollen Multi-Prozess-Flow; der schmale manuelle PumpSwap-E2E-Slice oben deckt nur den genannten Wire-Vertrag ab. Andere Cold-Path-Klassifikationen oder interne Priorisierung bleiben ausserhalb dieses Eval-Schnitts, solange nicht separat spezifiziert.
- **Getestet:** i24e_force_refresh_skips_stale_livepool_cache_pool_accounts; i24e_force_refresh_refuses_without_cold_path_rpc_permission; i24e_pool_address_hint_parse_fail_errors_without_global_scan; control_request_ensure_pump_amm_pool_accounts_force_refresh_and_pool_hint_roundtrip; request_reply_e2e_manual_pumpswap_sell_all_pool_hint_roundtrip.
- **Kontext:** PR #31 Merge auf architecture-rebuild; PumpSwap Cold-Path Hint/Recovery auf Request/Reply (PR #65, verfeinert PR #67 architecture-rebuild: bounded Ensure vor `build_tx_plan` fuer den narrow Shape oben); Hot-Path-async-Refresh fuer reguläre Sells bleibt explizit out-of-scope (kein Overclaim in diesen Tests).
...
- **PR233 (Evolution, Impl PR #233):** `schedule_geyser_sync_batch_debounced` enqueued bounded `MdStateCommand::FlushGeyserSyncDebounced` auf `md-state` — **kein** inline `sync_geyser_tracked_accounts_batched_flush` im Tokio-/Debounce-Pfad.
- **Phase 2a Hybrid (Evolution, Impl PR #239, Plan §2a/§5.2):** Geyser explicit-set mutations + coalesced delta-only push laufen auf dem `md-track-worker` OS-Thread (`DesiredExplicitSet` SSOT). `FlushGeyserSyncDebounced` wird in `md_state_process_job` an den Track-Worker weitergeleitet (`track_worker_try_enqueue` / `ScheduleGeyserPushDebounced`); `sync_geyser_tracked_accounts_batched_flush` (oder `_with_deadline`) laeuft in `track_worker_execute_coalesced_push`, **nicht** mehr in `md_state_worker_loop`. TX/Account-Ingest ruft `sync_geyser_tracked_accounts_batched_flush` weiterhin **nicht** inline auf.
- **Phase 2b Hybrid (Evolution, Plan §2b):** Momentum Active Pools (`ironcrab.v1.momentum.active_pools`) duerfen **nicht** ueber die md-state-Job-Queue laufen (`MdStateCommand::ApplyMomentumActivePools` + `md_state_try_enqueue` verboten im NATS-/Coalescer-Pfad; kein `apply_momentum_active_pools_in_md_state` aus dem Momentum-Handler). Pin/Unpin und explicit-set mutation fuer Momentum laufen auf `md-track-worker` via bounded `track_worker_try_enqueue` + `TrackWorkerCommand::ApplyMomentumActivePools`; `track_worker_process_job` fuehrt den coalesced Geyser push aus (Phase-2a-Vertrag bleibt). `MdStateCommand::ApplyMomentumActivePools` entfaellt in `md_state_process_job`.
- **Phase 3 Hybrid (Evolution, Plan §8 / I-4e):** Arb Track Requests (`ironcrab.v1.arb.track_requests`) — siehe **A.47**; symmetrisch Phase 2b auf `md-track-worker`, kein md-state-Pfad.
- **Phase 1 Hybrid (Evolution, Impl PR #238, Plan §5.2/§5.4):** Ingest `might_be_relevant()` und Sidefx-Vault-Ticks nutzen `SnapshotView` / `TrackedMembershipSnapshot` — **kein** `tracked_vaults.read()`, `tracked_mints.read()`, `tracked_bin_arrays.read()` in Ingest-/Sidefx-Modulpfaden (`account_geyser_update_might_be_relevant`, `account_geyser_dispatch_priority_high`, `handle_geyser_transaction`, `md_sidefx_process_vault_balance_tick`, `md_sidefx_flush_pending_md_state_jobs`). Sidefx/TX-parse enqueued **kein** `RegisterReservesAfterTrade` oder `RegisterPoolVaultsFromAccount` aus Account-/Trade-Parse (Registrierung nur noch auf `md-state`).
- **Source-Contract (PR233 + Phase 2a/2b/3):** `tests/invariants_market_data_tracking_single_writer.rs` — `md_state_command_includes_flush_geyser_sync`; `phase2a_geyser_sync_on_track_worker_not_md_state_loop`; `phase2b_momentum_active_pools_bypasses_md_state`; `phase3_arb_track_requests_bypasses_md_state`; `phase3_arb_track_requests_uses_track_worker`; `global_ingest_liveness_os_thread`; `touch_tracked_vault_o1_contract`.
- **Source-Contract (Phase 1 + Phase 2c):** `tests/invariants_market_data_i4b_ingest_no_tracked_read.rs` — `phase1_ingest_sidefx_no_tracked_map_reads`; `phase1_trade_path_no_register_reserves`; `phase1_sidefx_no_register_pool_from_account`; `phase1_trade_path_no_arb_reconcile_enqueue`; `phase2c_no_arb_multidex_reconcile_in_market_data`.
- **Getestet:** `i4b_jsonl_full_queue_try_enqueue_json_returns_immediately`; `i4b_jsonl_full_queue_try_enqueue_market_event_returns_immediately`; `i4b_jsonl_bounded_enqueue_delivers_when_capacity_available`; PR233 + Phase-2a/2b Source-Contract in `invariants_market_data_tracking_single_writer.rs`; Phase-1 + Phase-2c Source-Contract in `invariants_market_data_i4b_ingest_no_tracked_read.rs`.
- **Kontext:** `plan_market_data_ingest_rebuild.md` Phase R1 (JSONL `QueuedJsonlMsg::MarketEvent`) + R2 (`md-state` Thread) + PR233 (Debounce → md-state enqueue) + Phase 2a/2b Hybrid (`plan_hybrid_rollback_tracking_architecture_20260623.md` §2a/§2b Track-Worker, §5.2 Snapshot-Ingest) + Phase 1 Hybrid (§5.4 Register-Verbot parse-Pfad) + Phase 2c Hybrid (§8 Arb-Reconcile-Entkopplung).

### A.46 Market-Data Ingest: kein ArbMultiDex-Reconcile (I-4c)
- **Datei:** `tests/invariants_market_data_i4b_ingest_no_tracked_read.rs`
- **Invariante (Phase 2c Hybrid, Plan §8):** Das `market_data`-Ingest-Modul importiert/ruft **keinen** Arb-Reconcile-Pfad auf — weder `reconcile_arb_*`, noch `hot_pool_registry` Arb-Reconcile-Enqueue, noch `MdStateCommand::ArbMultiDexReconcile`.
- **Source-Contract gegen Sibling `Iron_crab/src/bin/market_data.rs`:**
  1. **Kein** `MdStateCommand::ArbMultiDexReconcile` in der `MdStateCommand`-Enum.
  2. **Keine** Funktionsdefinitionen `try_enqueue_arb_reconcile_for_pool`, `try_enqueue_arb_multi_dex_reconcile`, `reconcile_arb_multi_dex_for_mint`, `reconcile_arb_multi_dex_for_pool`.
  3. Trade-Pfad (`handle_geyser_transaction` oder aequivalent): **kein** Aufruf von `try_enqueue_arb_reconcile*` oder `reconcile_arb_multi_dex*`.
- **Erlaubt:** Kommentare, historische `BUGS_FIXES`-Strings — **nicht** ausfuehrbarer Code (Grep prueft Definitionen und Aufruf-Marker, nicht reine String-Literale in Kommentaren).
- **Getestet:** `phase1_trade_path_no_arb_reconcile_enqueue`; `phase2c_no_arb_multidex_reconcile_in_market_data`.
- **Kontext:** Impl PR #241 Phase 2c auf `architecture-rebuild`; ergaenzt A.45 Phase-1 Arb-Enqueue-Verbot um globales Phase-2c-Loesch-Gate.

### A.47 Arb Track Requests NATS (I-4e, Phase 3 Hybrid)
- **Dateien:** `tests/invariants_arb_track_requests.rs`, `tests/invariants_market_data_tracking_single_writer.rs`, `docs/spec/ARB_TRACK_REQUESTS.md`
- **Invariante (Plan §8, Phase 3 Arb):** Nur `momentum_bot` / `arb_strategy` / `execution_engine` duerfen Track-Request-**topics** publizieren (Schema-Test Gesamtarchitektur). Fuer **`ironcrab.v1.arb.track_requests`** gilt Phase 3 konkret:
  1. **Publish** (`nats.publish(` + `TOPIC_ARB_TRACK_REQUESTS`): **nur** in `src/bin/arb_strategy.rs` (genau eine Publish-Stelle).
  2. **Subscribe** (`subscribe(TOPIC_ARB_TRACK_REQUESTS)`): **nur** in `src/bin/market_data.rs`.
  3. **Verboten:** publish in `market_data.rs`, `momentum_bot.rs`, `execution_engine.rs` fuer dieses Topic; subscribe in `arb_strategy.rs`.
- **MD Track-Worker Pfad (symmetrisch Phase 2b):** NATS arb subscriber / coalescer (`spawn_arb_tracking_coalescer`) enqueued **`track_worker_try_enqueue`** + `TrackWorkerCommand::ApplyArbTrackRequests`; `track_worker_process_job` wendet Pins an. **Verboten:** `md_state_try_enqueue` + `MdStateCommand::ApplyArbTrackRequests` (Variante darf in `MdStateCommand` nicht existieren; `md_state_process_job` darf sie nicht handeln).
- **Schema (Blackbox):** `ArbTrackRequestsUpdate` JSON roundtrip gegen oeffentliche `ironcrab::nats` Types; Spec-Sample in `ARB_TRACK_REQUESTS.md`.
...
- **Datei:** `tests/invariants_arb_quote_contract.rs` (M1: E-ARB-1; M2: E-ARB-2 geplant)
