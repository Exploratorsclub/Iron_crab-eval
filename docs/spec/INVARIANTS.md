...
- **Source-Contract (Phase 1):** `tests/invariants_market_data_i4b_ingest_no_tracked_read.rs` — `phase1_ingest_sidefx_no_tracked_map_reads`; `phase1_trade_path_no_register_reserves`; `phase1_sidefx_no_register_pool_from_account`; `phase1_trade_path_no_arb_reconcile_enqueue`.
- **Getestet:** `i4b_jsonl_full_queue_try_enqueue_json_returns_immediately`; `i4b_jsonl_full_queue_try_enqueue_market_event_returns_immediately`; `i4b_jsonl_bounded_enqueue_delivers_when_capacity_available`; PR233 + Phase-2a/2b Source-Contract in `invariants_market_data_tracking_single_writer.rs`; Phase-1 Source-Contract in `invariants_market_data_i4b_ingest_no_tracked_read.rs`.
- **Kontext:** `plan_market_data_ingest_rebuild.md` Phase R1 (JSONL `QueuedJsonlMsg::MarketEvent`) + R2 (`md-state` Thread) + PR233 (Debounce → md-state enqueue) + Phase 2a/2b Hybrid (`plan_hybrid_rollback_tracking_architecture_20260623.md` §2a/§2b Track-Worker, §5.2 Snapshot-Ingest) + Phase 1 Hybrid (§5.4 Register-Verbot parse-Pfad).
